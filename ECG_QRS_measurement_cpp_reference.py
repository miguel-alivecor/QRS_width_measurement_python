...
def ECG_QRS_measurement(beat_avg, medianRR, Fs = 300, log=print):
    ...
    try:
        ...
        if np.min(aux[3:10]) > -5e-3:  # QS morphology check; if true, J-point ~= peak index. Search finished.
            ...
            inxTot = 0
        else: # Most general case. Start search from R-wave peak.
            log('MeasureQRSWidth J-Point.3')
            ...
            if len(inx1) == 0:
                log('MeasureQRSWidth J-Point.3.1')
                return ('QRS measurement error.', avg_beat_features)
            ...
            if (np.min(aux[1:6]) < -0.0086 and np.max(aux[:5]) < 0.058 and inx_aux <= 50) or np.min(aux[:6]) < -0.015:
                log('MeasureQRSWidth J-Point.3.2')
                aux_tmp = aux
                inxTotCpy = inxTot
                ...
                if inx < 14 and inx1[0] < 51:
                    log('MeasureQRSWidth J-Point.3.2.1')
                    inxTot += inx
                    aux = aux[inx:]
                ...
                if inx == 0 and inx2[0] == inx: # rollback
                    log('MeasureQRSWidth J-Point.3.2.2')
                    aux = aux_tmp
                    inxTot = inxTotCpy
                    slur_flag = False
            ...
            if norm_coeff > 0.79:
                thresh1 = 0.00634
                log('MeasureQRSWidth J-Point.3.3 thresh1: %s' % thresh1)
            else:
                thresh1 = 0.0061
                log('MeasureQRSWidth J-Point.3.4 thresh1: %s' % thresh1)
            if np.max(aux[:9]) > thresh1 and not slur_flag:  # additional non-negligible slope increase
                log('MeasureQRSWidth J-Point.3.5')
                aux_tmp = aux
                inxTotCpy = inxTot
                ...
                if norm_coeff > 0.21:
                    thresh1 = 0.031
                    log('MeasureQRSWidth J-Point.3.5.1 thresh1: %s' % thresh1)
                else:
                    thresh1 = 0.0184
                    log('MeasureQRSWidth J-Point.3.5.2 thresh1: %s' % thresh1)
                ...
                if inx < 2:
                    log('MeasureQRSWidth J-Point.3.5.3')
                    if norm_coeff > 0.74:
                        thresh1 = 1e-2
                        log('MeasureQRSWidth J-Point.3.5.3.1 thresh1: %s' % thresh1)
                    elif norm_coeff > 0.57:
                        thresh1 = 7e-3
                        log('MeasureQRSWidth J-Point.3.5.3.2 thresh1: %s' % thresh1)
                    else:
                        thresh1 = 5e-3
                        log('MeasureQRSWidth J-Point.3.5.3.3 thresh1: %s' % thresh1)
                ...
                if inx > 12:  # restart search with a higher detection threshold; overrule previous inx
                    log('MeasureQRSWidth J-Point.3.5.4')
                    ...
                    if inx > 10:  # restart search with a higher detection threshold; overrule previous inx
                        log('MeasureQRSWidth J-Point.3.5.4.1')
                        high_slope_flag = True
                        ...
                        if inx > 11:  # restart search with a higher detection threshold; overrule previous inx
                            log('MeasureQRSWidth J-Point.3.5.4.1.1')
                            high_slope_flag = True
                            inx = np.where(np.abs(aux) <= 5e-1)[0]
                            if len(inx) == 0:
                                log('MeasureQRSWidth J-Point.3.5.4.1.1.1')
                                inx = np.Inf
                            else:
                                inx = inx[0]
                ...
                if (inx == 0 and slur_flag == False) or (inx == 0 or inx == 1 and high_slope_flag == True):  # rollback
                    log('MeasureQRSWidth J-Point.3.5.5')
                    high_slope_flag = False
                    aux = aux_tmp
                    inxTot = inxTotCpy
            ...
            if np.min(aux[6:15]) < -4.5e-2:
                log('MeasureQRSWidth J-Point.3.6')
                inxTotCpy = inxTot
                ...
                inx = int(np.min([inx1[0], inx2[0]]))
                if inx < 60:
                    log('MeasureQRSWidth J-Point.3.6.1')
                    inxTot += inx
                    aux = aux[inx:]
                else:  # rollback
        ...
        if QS_flag and np.min(aux[:3]) > -0.038: # QS-like morphology; use current index as QRS onset; search finished.
            ...
            if inx0 > 43 or (beat_wave_lpf_norm[qrs_peak_inx] < 0 and inx0 == inx0_1): # QS type
            ...
            if norm_coeff > 0.65:
                ...
                thresh1 = 0.01
                log('MeasureQRSWidth QRS onset.3.4 thresh1: %s' % thresh1)
            ...
            if np.max(aux[:12]) >= thresh1: # look for Q deflection
                ...
                if (inx <= 19 and np.max(aux[inx5 + 1:inx5 + 1 + 5]) < 0.0125 and np.min(aux[inx5:inx5 + 18]) > -6.3e-3 and inx5 > 1) or \
            ...
            # elif np.min(aux[1:12]) < -0.0176 and np.max(aux[2:15]) < 6e-4 and norm_coeff > 0.40:  # slurry QRS
                ...
                if inx[0] > 15 and np.min(aux[:20]) > -9.3e-3: # <---- v2
                    log('MeasureQRSWidth QRS onset.4.2')
                    aux = aux_tmp
                    inxTot = inxTotCpy
